...
static void drainEncoder(AVCodecContext *ctx, const AVStream *stream,
                         AVFormatContext *fmt, const AVFrame *frame) {
  if (!ctx || !stream || !fmt)
    return;
...
/** close() 時に FIFO に残った音声サンプルをエンコードして書き出す。*/
void FFmpegMuxer::Impl::flushRemainingAudioFifo() {
  const int frameSize =
      audioCtx->frame_size > 0 ? audioCtx->frame_size : 1024;
  const int remaining = av_audio_fifo_size(audioFifo);
  if (remaining <= 0)
    return;

  const bool canSmall =
      (audioCtx->codec->capabilities & AV_CODEC_CAP_SMALL_LAST_FRAME) != 0;
  const int toEncode = canSmall ? remaining : frameSize;

  av_frame_unref(audioFrame);
  audioFrame->nb_samples = toEncode;
  audioFrame->format = audioCtx->sample_fmt;
  audioFrame->sample_rate = audioCtx->sample_rate;
  av_channel_layout_copy(&audioFrame->ch_layout, &audioCtx->ch_layout);
  av_frame_get_buffer(audioFrame, 0);

  std::array<void *, AV_NUM_DATA_POINTERS> planes;
  for (int i = 0; i < AV_NUM_DATA_POINTERS; ++i)
    planes[static_cast<size_t>(i)] = audioFrame->data[i];
  av_audio_fifo_read(audioFifo, planes.data(), remaining);

  audioFrame->pts = audioPts;
  audioPts += toEncode;
  drainEncoder(audioCtx, audioStream, fmtCtx, audioFrame);
}
...
bool FFmpegMuxer::open(const Settings &settings) {
  ...
  // ── audio stream ──────────────────────────────────────────────
  {
    ...
    AVCodecContext *ac = impl->audioCtx;
    {
      ...
      int nFmts = 0;
      if (avcodec_get_supported_config(nullptr, acodec,
                                       AV_CODEC_CONFIG_SAMPLE_FORMAT, 0,
                                       reinterpret_cast<const void **>(&fmts), // NOSONAR - required by avcodec_get_supported_config (FFmpeg C API)
                                       &nFmts) == 0 && nFmts > 0)
...
void FFmpegMuxer::writeVideoFrame(const uint8_t *pixelData, int stride,
  ...
  const std::array<const uint8_t *, 4> srcData = {pixelData, nullptr, nullptr, nullptr};
  const std::array<int, 4> srcLinesize = {stride, 0, 0, 0};

  sws_scale(impl->swsCtx, srcData.data(), srcLinesize.data(), 0, srcHeight,
...
void FFmpegMuxer::writeAudioSamples(const juce::AudioBuffer<float> &buffer,
  ...
  std::array<const uint8_t *, 8> inPtrs = {}; // NOSONAR - uint8_t* required by swr_convert (FFmpeg C API)
  for (int ch = 0; ch < numChannels; ++ch)
    inPtrs[static_cast<size_t>(ch)] = reinterpret_cast<const uint8_t *>(buffer.getReadPointer(ch)); // NOSONAR
  ...
  tmp->nb_samples =
      swr_convert(impl->swrCtx, tmp->data, tmp->nb_samples, inPtrs.data(), numSamples);

  {
    std::array<void *, AV_NUM_DATA_POINTERS> planes;
    for (int i = 0; i < AV_NUM_DATA_POINTERS; ++i)
      planes[static_cast<size_t>(i)] = tmp->data[i];
    av_audio_fifo_write(impl->audioFifo, planes.data(), tmp->nb_samples);
  }
  av_frame_free(&tmp);
  ...
  while (av_audio_fifo_size(impl->audioFifo) >= frameSize) {
    ...
    impl->audioFrame->nb_samples = frameSize;
    {
      std::array<void *, AV_NUM_DATA_POINTERS> planes;
      for (int i = 0; i < AV_NUM_DATA_POINTERS; ++i)
        planes[static_cast<size_t>(i)] = impl->audioFrame->data[i];
      av_audio_fifo_read(impl->audioFifo, planes.data(), frameSize);
    }

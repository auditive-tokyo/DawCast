...
public:
  ...
  enum class CaptureMode {
    ...
    /**
     * ユーザー指定の矩形領域。
     * SCStreamConfiguration.sourceRect に CGRect
     * を渡してトリミング（macOS 14.2+）。 IPC: "region" （未実装 — RecorderApp
     * の UI 選択フロー完成後に対応予定）
     */
    CustomRegion,
  };
   ...
   * sampleBuffer の lifetime はコールバック内のみ保証。保持する場合は CFRetain
   * する。
   */
  using FrameCallback = std::function<void(CMSampleBufferRef sampleBuffer,
  ...
  bool startRecording(CaptureMode mode = CaptureMode::EntireDisplay,
                      int fps = 60, int width = 0, int height = 0,
   ...
   * 権限がない場合は macOS の許可ダイアログ（または System Settings
   * へ誘導）を出す。 YES = 既に権限あり。NO = ダイアログを出した（=今回の REC
   * はキャンセル、再試行が必要）。
   */
